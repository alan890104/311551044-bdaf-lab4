...
contract Safe is Ownable {
    ...
    // We should use nonReentrant modifier to prevent reentrancy attack
    function withdraw(
        address token,
        uint256 amount
    ) external nonZeroAmount(amount) {
    ...
    function getFee(address token) external view onlyOwner returns (uint256) {
        return _fees[token];
    }

    // takeFee function allows owner to take the accumulated fees for a token
    function takeFee(address token) public onlyOwner {
